@extends('layouts.app', ['page' => 'List of TransactionType', 'pageSlug' => 'TransactionType', 'section' => 'setings'])
            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-4 text-right">
                        <a href="{{ route('transactionType.create') }}" class="btn btn-sm btn-primary">New TransactionType</a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="td-actions">
                                    ...
                                    <form action="{{ route('transactionType.destroy', $model->id) }}" method="post" class="d-inline">
                                        @csrf
                                        @method('delete')
...
<script>
    ...
    function changeStatus(_this,id) {
           ...
           $.ajax({
               url: `changeTransactionTypeStatus`,
               type: 'post',
               data: {
                   _token: _token,
                   id: id,
    ...
    function splitData(type) {
        ...
        $.ajax({
            url: "{{ route('getTransactionTypeSplitedData') }}",
            type: "post",
            data: type,
            data: {
                _token: '{{ csrf_token() }}',
            ...
            success: function(response) {
                    ...
                    $.each(Result, function(key, value) {
                        ...
                        var editurl = '{{ route('transactionType.edit', ':id') }}';
                        editurl = editurl.replace(':id', value.id);

                        var deleteurl = '{{ route('transactionType.destroy', ':id') }}';
                        deleteurl = deleteurl.replace(':id', value.id);

                        var row = `<tr role="row" class="odd"><td>` + (key + 1) + `</td><td>` + value
                            .transaction_type_name + `</td><td>` + value
                            .gen_name + `</td><td>` + value
                            .gen_no + `</td><td><label class="switch"> <input type="checkbox" onchange="changeStatus(event.target,`+value.id+`)" `+StatusRes+` ><span class="slider round"></span></label></td><td class="td-actions"><a href ="` + editurl +
