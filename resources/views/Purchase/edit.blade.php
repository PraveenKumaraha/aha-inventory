				...
				<div class="card">
					<div class="card-header">
						<div class="row align-items-center">
							<div class="col-8">
								<h3 class="mb-0"style="margin: 0; display: inline;">Edit Purchase </h3>#<h4 class ="referenceNo" style="margin: 0; display: inline;color:blue" align="right"><?php echo $model->reference_no;?></h4>
						...
						<div class="container-sm" style="width:100%;border-style: solid;border-color: coral;">
							...
							<div class="form-row">
								...
								<select class="form-control col-sm-2 " id="supplierselect" name="supplier">
									...
									<?php foreach ($pdtsupplierIds as $row) { ?> <option value="<?php echo ($row["id"]);?>" <?php echo($row["id"]==$model->supplier_id)?  'selected':'';?>><?php echo ($row["supplier_id"]); ?> | <?php echo ($row["supplier_name"]); ?></option>
									<?php } ?>
								</select>
							...
							<div class="form-row"> -->
								...
								<input type="date" class="form-control col-sm-2" name="date" id="name" value="{{$model->date}}"/>
							</div>
						</div>
						<table class="table table-bordered item-table" id="productTable">
							...
							<tbody>
								<div class="col-sm-12 rowInvoice1">
                                    <?php for($i=0;$i<count($purchaseItems);$i++){?>
									<tr class="<?php echo $i+1;?>" id="row<?php echo $i+1;?>">

										<td data-select2-id="1">
											<div class="form-group">
												<select class="form-control	select2 productName<?php echo $i+1;?>" style="width:150px;height:80px!important" id="productName<?php echo $i+1;?>" name="product_name[]" onchange="getProductData(<?php echo $i+1;?>)" required>
													<option value="0" selected="selected" disabled>Select Product</option>
													<?php foreach ($pdtproductIds as $row) { ?> <option value="<?php echo ($row["id"]); ?>"  <?php echo($row["id"]==$purchaseItems[$i]->item_id)?  'selected':'';?> a_price="<?php echo ($row["a_price"]); ?>"><?php echo ($row["product_id"]); ?> | <?php echo ($row["product_name"]); ?></option>
													<?php } ?>
												</select>
											</div>
										</td>
										<td>
											<input type="number" name="qty[]" id="quantity<?php echo $i+1;?>" min="1" class="form-control quantity<?php echo $i+1;?>" onkeyup="getQtyData(<?php echo $i+1;?>)"  value="<?php echo($purchaseItems[$i]->quantity);?>">
										</td>
										<td>
											<input type="number" name="rate[]" id="rate<?php echo $i+1;?>" class="form-control rate<?php echo $i+1;?>" onkeyup="getRateData(<?php echo $i+1;?>)" value="<?php echo($purchaseItems[$i]->rate);?>">
										</td>
										<td data-select2-id="1">
											<div class="form-group">
												<select class="form-control	select2 tax<?php echo $i+1;?>" style="width:150px;height:80px!important" id="tax<?php echo $i+1;?>" name="tax[]" onchange="getTaxData(<?php echo $i+1;?>)">
													...
													<?php foreach ($pdttaxids as $row) { ?> <option value="<?php echo ($row["id"]); ?>" <?php echo($row["id"]==$purchaseItems[$i]->tax_id)?  'selected':'';?> taxValue="<?php echo ($row["tax_value"]); ?>"><?php echo ($row["tax_name"]); ?></option>
													<?php } ?>
												</select>
											</div>
										</td>
										...
										<td>
											<input type="number" name="total[]" id="total<?php echo $i+1;?>" class="form-control" value="<?php echo($purchaseItems[$i]->final_amount);?>" disabled>
										</td>
										<td>

											<a class="delete" title="Delete" data-toggle="tooltip" onclick="removeProductRow(<?php echo $i+1;?>)"><i class="material-icons" style="color:red">&#xE872;</i></a>
										</td>
									</tr>
                                    <?php }?>
								</div>
